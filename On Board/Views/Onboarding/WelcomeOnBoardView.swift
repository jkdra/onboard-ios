        ...
        NavigationStack {
            ...
            .navigationDestination(isPresented: $showPledge) {
                PledgeSignatureView {
                    ...
                    // Same fire-and-forget shape: the signed pledge doubles as
                    // agreement to the current terms/privacy versions, recorded
                    // via accept_legal_documents (no-op in mock builds).
                    Task { await LegalService.acceptCurrentDocuments() }
                    dismiss()
                }
            }
        }
