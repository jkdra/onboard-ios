...
- **Changing onboarding steps**: `OnboardingStatus.swift` defines the step enum; `OnboardingStore.swift` drives the state machine. Onboarding is **5 pushes**: birthday → username → profile → school → waitlist. The school screen (`OnboardingSchoolEmailStepView`) is a three-stage machine — email → code (the OTP entry *replaces* the email field; "Use a different email" is the way back) → details (graduation pickers + the profanity preference). `.graduation` still exists as a client-only hold state with **no backing value in the `onboarding_step` Postgres enum** (`birthday`/`username`/`profile`/`school_verify`/`waitlist`/`complete` only), but `targetPath(for: .graduation)` deliberately equals `targetPath(for: .schoolVerify)` so verification advances the stage in place rather than pushing — `graduationHoldsOnTheSchoolScreen` pins this. The old standalone `.contentPreferences` and graduation screens were merged away 2026-08-07/08. `profanityEnabled` remains deliberately plain, un-scoped `@AppStorage` (per-device, not per-account). Don't scope it to the signed-in user; that's not a bug, it's the documented design.
