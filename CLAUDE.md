...
- **Changing onboarding steps**: `OnboardingStatus.swift` defines the step enum; `OnboardingStore.swift` drives the state machine. `.graduation` is a client-inserted step with **no backing value in the `onboarding_step` Postgres enum** (`birthday`/`username`/`profile`/`school_verify`/`waitlist`/`complete` only). Its screen also hosts the profanity preference — the old standalone `.contentPreferences` step (a full push for one toggle) was merged into it 2026-08-07, taking onboarding from 7 pushes to 6. `profanityEnabled` remains deliberately plain, un-scoped `@AppStorage` (per-device, not per-account). Don't scope it to the signed-in user; that's not a bug, it's the documented design.
