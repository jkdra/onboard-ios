...
### Optimistic Updates
The `BoardStore+Posts/+Comments/+Reactions/+Moderation` extensions (formerly one `BoardStore+Interactions.swift`) apply all mutations locally first, then sync to Supabase. On error it rolls back and surfaces the error. Never skip the rollback path when adding new mutations.
...
Non-push cron jobs also live (verified 2026-08-08): `rotate-board-weeks` (hourly), `delete-unused-tags-3d` (daily 03:00), and `moderation-drain` (every minute — drains the pgmq `moderation_queue` into the `moderate-content` Edge Function's LLM judge).
...
- **Remote config lives in `Configuration/RemoteConfig.swift` and nowhere else.** It is the only file containing config key literals; call sites read typed accessors (`config.feedPollSeconds`), never strings. Every accessor falls back to the value that was compiled in, so a server returning `{}` behaves exactly like a build with no remote config — never add an accessor whose fallback makes the caller skip behavior it does today. Adding a key means: add it to the `get_app_config` allowlist migration, add a typed accessor with the current hardcoded value as its fallback, then read it at the call site. **Forgetting the allowlist half fails silently** — the key is filtered out server-side, the accessor returns its default forever, and nothing errors anywhere. This happened with `enabled_reactions` on 2026-08-05. Verify a new key end to end with `insert into app_config ...; select public.get_app_config();` before trusting it; seeding `UserDefaults` directly (useful for mock-mode visual checks) bypasses the RPC and will not catch this. The RPC uses an explicit allowlist, **never `select *`** — `app_config` also holds server-only keys (`board_timezone` drives the cron reset) that would otherwise leak to anyone holding the shipped anon key.
...
- **Recreating a Postgres function silently widens its access.** Supabase's default privileges grant `EXECUTE` on new `public`-schema functions to `anon`, and `revoke ... from public` does **not** undo an explicit role grant. So a `drop function` + `create function` (needed whenever the parameter list changes) can hand `anon` a function that was authenticated-only before. This happened to `register_device_token` on 2026-08-02 while adding `p_app_version`: the body upserts on `(token)` and writes `auth.uid()`, so an unauthenticated caller who knew a device token could have nulled its `user_id` and unregistered that device from push. Always follow a recreate with an explicit `revoke execute ... from anon` and verify with `has_function_privilege('anon', 'public.fn(args)', 'execute')` — the Supabase security advisor also flags it as `anon_security_definer_function_executable`.
- **Guard `SECURITY DEFINER` RPCs in the body, not only with the grant.** `accept_pledge`, `accept_legal_documents`, and `set_expected_graduation` all open with `if auth.uid() is null then raise exception 'Not authenticated'; end if;` — which is why their (also accidental) `anon` grant is harmless. `register_device_token` had no such guard, so the grant was its only protection and a signature change silently removed it. It now carries the same guard. A new RPC that writes `auth.uid()` should have both: the grant is the lock, the guard is what survives someone changing the door.
...
- **Changing onboarding steps**: `OnboardingStatus.swift` defines the step enum; `OnboardingStore.swift` drives the state machine. Onboarding is **6 pushes**: birthday → username → profile → school → profanity → waitlist. The school screen (`OnboardingSchoolEmailStepView`) is a three-stage machine — email → code (the OTP entry *replaces* the email field; "Use a different email" is the way back) → graduation. Two client-only steps have **no backing value in the `onboarding_step` Postgres enum** (`birthday`/`username`/`profile`/`school_verify`/`waitlist`/`complete` only): `.graduation` is a hold state whose `targetPath` deliberately equals `.schoolVerify`'s, so verification advances the school screen's stage in place rather than pushing (`graduationHoldsOnTheSchoolScreen` pins this); `.contentPreferences` is a real pushed screen inserted between graduation and waitlist, gated on the local `hasCompletedProfanityStep` flag — and only when the backend says `.waitlist`, so a mid-flow admin approval wins over the toggle. `profanityEnabled` remains deliberately plain, un-scoped `@AppStorage` (per-device, not per-account). Don't scope it to the signed-in user; that's not a bug, it's the documented design.
...
7. **Security advisors** — run the Supabase security linter and triage anything new. Two known, accepted findings as of 2026-07-10:
   ...
   - `rls_enabled_no_policy` on `admin_audit_log`, `admin_users`, `push_notification_ledger`, `waitlist` — and (added since) `archived_reported_posts`, `moderation_flags`, `moderation_log`, `prompt_queue`, `school_domain_requests`, `user_reaction_counts` — is intentional: RLS on with zero policies denies `anon`/`authenticated` outright; those tables are reached only through `SECURITY DEFINER` RPCs and `service_role`.
   - `get_invite_inviter` is `SECURITY DEFINER` and anon-executable **on purpose**: the web `/invite/<code>` page resolves the inviter's handle pre-auth. An 8-char code over a 32-symbol alphabet makes enumeration impractical; accept, don't revoke.
